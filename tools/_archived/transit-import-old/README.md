...
>
> These scripts target the **deleted** `import_transport.*` / `core_transport.*` / `gtfs_export.*` schemas and the legacy `core.core_bus_*` tables. Those database objects no longer exist.
>
> The current transport data lives in `transport.*` (and `transit_export.*` for future GTFS/OTP metadata). Future transport imports will be a simple direct-upsert into `transport.*` — **not** the staging/promotion design below.
>
