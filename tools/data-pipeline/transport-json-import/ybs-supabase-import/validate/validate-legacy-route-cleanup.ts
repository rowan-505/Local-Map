...
import { YBS_SOURCE_NAME } from "../lib/source-link-utils.js";
...
function printHelp(): void {
    console.log(`Usage:
  npx tsx tools/data-pipeline/transport-json-import/ybs-supabase-import/validate/validate-legacy-route-cleanup.ts \\
...
function renderMarkdown(report: ValidationReport): string {
    ...
    if (report.result === "PASS") {
        lines.push(
            ...
            "",
            "```bash",
            "npx tsx tools/data-pipeline/transport-json-import/ybs-supabase-import/cleanup/report-orphan-legacy-stops.ts \\",
            "  --report-dir tmp/transport-imports/legacy-cleanup",
            "```",
            "",
        );
...
function buildNextAction(result: "PASS" | "FAIL", expectZero: boolean, legacyCount: number): string {
    if (result === "PASS") {
        return "PASS — proceed to Phase D2 orphan stop report: npx tsx tools/data-pipeline/transport-json-import/ybs-supabase-import/cleanup/report-orphan-legacy-stops.ts --report-dir tmp/transport-imports/legacy-cleanup";
    }
