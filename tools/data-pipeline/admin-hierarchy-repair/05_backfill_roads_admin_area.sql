...
-- (use run_05_roads_loop.sh to loop automatically).
--
...
--
-- Prerequisite: 03_create_admin_assignment_functions.sql
-- =============================================================================
...
BEGIN
    IF to_regprocedure('core.find_admin_area_for_line(geometry,text)') IS NULL THEN
        RAISE EXCEPTION 'Run 03_create_admin_assignment_functions.sql first';
    END IF;
    IF to_regprocedure('core.admin_area_row_matches_target(bigint,text,text,text)') IS NULL THEN
        RAISE EXCEPTION 'Run 03_create_admin_assignment_functions.sql first (level helpers)';
    END IF;
END $$;
