#!/usr/bin/env bash
# Loop backfill_streets_admin_area_id.sql until done=true (one chunk per psql call).
#
# Usage:
#   DRY_RUN=true ./run_backfill_streets_admin_area_id_loop.sh imports/template.env
#   CONFIRM_WRITE=true ./run_backfill_streets_admin_area_id_loop.sh imports/template.env
#   LIMIT_ROWS=5000 LAST_ID=0 DRY_RUN=true ./run_backfill_streets_admin_area_id_loop.sh imports/template.env
#
# Does NOT run on app startup — manual operator script only.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENV_FILE="${1:-imports/template.env}"
...
while true; do
  CHUNK_N=$((CHUNK_N + 1))
  log "=== streets admin backfill chunk #${CHUNK_N} last_id=${LAST_ID} dry_run=${DRY_RUN} limit_rows=${LIMIT_ROWS} ==="
  ...
  RESULT_LINE="$(
    psql "${DB_URL}" -v ON_ERROR_STOP=1 \
      ...
      -v last_id="${LAST_ID}" \
      -q -t -A -F',' \
      -f "${SCRIPT_DIR}/backfill_streets_admin_area_id.sql" 2>&1 | tee /dev/stderr | awk -F',' '/^[0-9]+,/ {print; exit}'
  )"

  if [[ -z "${RESULT_LINE}" ]]; then
    echo "error: could not parse chunk_result row from backfill output" >&2
    exit 1
  fi
  ...
  if [[ "${DONE}" == "t" || "${SCANNED}" == "0" ]]; then
    log "streets admin backfill finished: chunks=${CHUNK_N} scanned=${TOTAL_SCANNED} updated=${TOTAL_UPDATED} unchanged=${TOTAL_UNCHANGED} no_match=${TOTAL_NO_MATCH} invalid_existing=${TOTAL_INVALID} would_clear_invalid=${TOTAL_WOULD_CLEAR}"
    exit 0
  fi
done
