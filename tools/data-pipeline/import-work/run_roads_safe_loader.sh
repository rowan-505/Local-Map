#!/usr/bin/env bash
# Roads safe loader — contract-aware runner.
# Does not reseed national roads. Scoped batches only.
set -euo pipefail
...
usage() {
  cat <<'EOF'
usage: run_roads_safe_loader.sh --target local|production --batch-code <code> [options]
...
safe_loader_preflight "${TARGET}" "${MODE}" "roads" "${BATCH_CODE}" "${CONFIRMATION}"
...
LOG_FILE="${LOG_DIR}/roads_loader_${SAFE_LOADER_TARGET}_${SAFE_BATCH}_${MODE}_${RUN_TS}.log"

echo ""
echo "=== roads_safe_loader ==="
...
echo "log=${LOG_FILE}"
echo "note=National roads NOT authorized from small samples alone"
...
stdbuf -oL -eL psql "${SAFE_LOADER_DATABASE_URL}" \
  ...
  -f "${SCRIPT_DIR}/roads_safe_loader.sql" \
  2> >(
