#!/usr/bin/env bash
# Export Yangon downtown safe_new landuse from LOCAL staging → import_work.
# Does not write core.*.
#
# Default mode is dry_run (export + report only). Production writes need:
#   --target production --apply --confirmation 'PRELOAD landuse <batch_code>'
set -euo pipefail
...
CONFIRMATION=""
ENV_FILE=""
BATCH_CODE="${BATCH_CODE:-landuse_yangon_downtown_safe_2026_07_23}"
CLI_SNAPSHOT_ID=""
...
usage() {
  cat <<'EOF'
usage: yangon_downtown_landuse_preload.sh --target local|production [options]
...
Options:
  ...
  --confirmation TEXT    production apply: PRELOAD landuse <batch_code>
EOF
}
...
# Pilot identity must not inherit city-wide SNAPSHOT_* from the env file.
BATCH_CODE="${BATCH_CODE:-landuse_yangon_downtown_safe_2026_07_23}"
...
TMP_CSV="$(mktemp -t yangon_landuse_XXXXXX.csv)"
TMP_SQL="$(mktemp -t yangon_landuse_XXXXXX.sql)"
...
PAGER=cat psql "${LOCAL_DATABASE_URL}" -v ON_ERROR_STOP=1 \
  ...
  FROM staging.staging_landuse_candidates AS s
...
if [[ "${ROWS}" -le 0 || "${ROWS}" -gt 100 ]]; then
  safe_loader_die "unexpected export size ${ROWS} (want 1..100 for downtown landuse pilot)"
fi
...
DROP TABLE IF EXISTS yangon_downtown_landuse_export_raw;
CREATE TEMP TABLE yangon_downtown_landuse_export_raw (
    external_id text,
    classification text,
    canonical_name text,
    name_en text,
...
\\copy yangon_downtown_landuse_export_raw FROM '${TMP_CSV}' WITH (FORMAT csv, HEADER true);

DROP TABLE IF EXISTS yangon_downtown_landuse_export;
CREATE TEMP TABLE yangon_downtown_landuse_export AS
SELECT
    external_id,
    classification,
    canonical_name,
