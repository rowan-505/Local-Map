#!/usr/bin/env bash
# Export Kyauktan safe_new water polygons (snapshot 4) → import_work.
# Yangon downtown has 0 eligible water polygons; this small real area proves the loader.
# Does not write core.*.
#
# Default mode is dry_run (export + report only). Production writes need:
#   --target production --apply --confirmation 'PRELOAD water_polygons <batch_code>'
set -euo pipefail
...
CONFIRMATION=""
ENV_FILE=""
BATCH_CODE="${BATCH_CODE:-water_polygons_kyauktan_safe_2026_07_23}"
CLI_SNAPSHOT_ID=""
...
usage() {
  cat <<'EOF'
usage: kyauktan_water_polygons_preload.sh --target local|production [options]
...
Options:
  ...
  --confirmation TEXT    production apply: PRELOAD water_polygons <batch_code>
EOF
}
...
TMP_CSV="$(mktemp -t kyauktan_wpoly_XXXXXX.csv)"
TMP_SQL="$(mktemp -t kyauktan_wpoly_XXXXXX.sql)"
...
PAGER=cat psql "${LOCAL_DATABASE_URL}" -v ON_ERROR_STOP=1 \
  ...
  FROM staging.staging_water_polygon_candidates AS s
...
if [[ "${ROWS}" -le 0 || "${ROWS}" -gt 20 ]]; then
  safe_loader_die "unexpected export size ${ROWS} (want 1..20 for kyauktan water_polygons pilot)"
fi
...
DROP TABLE IF EXISTS kyauktan_water_polygon_export_raw;
CREATE TEMP TABLE kyauktan_water_polygon_export_raw (
...
\\copy kyauktan_water_polygon_export_raw FROM '${TMP_CSV}' WITH (FORMAT csv, HEADER true);

DROP TABLE IF EXISTS kyauktan_water_polygon_export;
CREATE TEMP TABLE kyauktan_water_polygon_export AS
SELECT
