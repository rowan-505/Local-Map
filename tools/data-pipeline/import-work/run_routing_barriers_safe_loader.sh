#!/usr/bin/env bash
# Routing barriers safe loader — contract-aware runner.
...
usage() {
  cat <<'EOF'
usage: run_routing_barriers_safe_loader.sh --target local|production --batch-code <code> [options]
...
Production apply only:
  --confirmation 'APPLY routing_barriers <batch_id>'
...
safe_loader_preflight "${TARGET}" "${MODE}" "routing_barriers" "${BATCH_CODE}" "${CONFIRMATION}"
...
LOG_FILE="${LOG_DIR}/routing_barriers_loader_${SAFE_LOADER_TARGET}_${SAFE_BATCH}_${MODE}_${RUN_TS}.log"

echo ""
echo "=== routing_barriers_safe_loader ==="
...
echo "log=${LOG_FILE}"
echo "note=Valhalla is NOT rebuilt by this loader"
...
stdbuf -oL -eL psql "${SAFE_LOADER_DATABASE_URL}" \
  ...
  -f "${SCRIPT_DIR}/routing_barriers_safe_loader.sql" \
  2> >(
