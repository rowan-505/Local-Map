-- =============================================================================
-- Water polygons set-based safe loader — body (no BEGIN/COMMIT).
-- Requires temp params: water_polygons_loader_params(batch_code, dry_run, sample_limit)
...
-- Names: core.core_map_water_polygon_names primary official en/mm (+ und).
-- =============================================================================

DO $$
BEGIN
    IF to_regclass('pg_temp.water_polygons_loader_params') IS NULL THEN
        RAISE EXCEPTION 'water_polygons loader body: water_polygons_loader_params missing';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM water_polygons_loader_params WHERE batch_code IS NOT NULL) THEN
        RAISE EXCEPTION 'water_polygons loader: batch_code is required';
    END IF;
    IF to_regclass('import_work.water_polygon_rows') IS NULL THEN
        RAISE EXCEPTION 'water_polygons loader: import_work.water_polygon_rows missing — apply migration 142';
    END IF;
    IF to_regprocedure('system.pipeline_osm_identity_key(text)') IS NULL THEN
        RAISE EXCEPTION 'water_polygons loader: system.pipeline_osm_identity_key missing — apply migration 137';
    END IF;
END $$;

DROP TABLE IF EXISTS water_polygons_loader_batch;
CREATE TEMP TABLE water_polygons_loader_batch (
...
INSERT INTO water_polygons_loader_batch
...
JOIN water_polygons_loader_params AS p ON p.batch_code = b.batch_code
WHERE b.entity_family = 'water_polygons';

DO $$
DECLARE
    b water_polygons_loader_batch%ROWTYPE;
...
BEGIN
    SELECT * INTO b FROM water_polygons_loader_batch;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'water_polygons loader: batch not found or not entity_family=water_polygons';
    END IF;
    SELECT dry_run INTO v_dry FROM water_polygons_loader_params;
    IF b.status NOT IN ('loaded', 'validated', 'applied', 'failed') THEN
        RAISE EXCEPTION 'water_polygons loader: batch status % not loadable', b.status;
    END IF;
    IF b.source_snapshot_id IS NULL OR nullif(btrim(b.source_snapshot_version), '') IS NULL THEN
        RAISE EXCEPTION 'water_polygons loader: batch missing snapshot identity';
    END IF;
    SELECT count(*),
    ...
    FROM import_work.water_polygon_rows AS r WHERE r.import_batch_id = b.import_batch_id;
    IF v_pmtiles > 0 THEN
        RAISE EXCEPTION 'water_polygons loader: % pmtiles_only row(s) in work table — refuse', v_pmtiles;
    END IF;
    IF b.loaded_row_count IS NOT NULL AND b.loaded_row_count <> v_actual THEN
        RAISE EXCEPTION 'water_polygons loader: loaded_row_count (%) <> actual (%)', b.loaded_row_count, v_actual;
    END IF;
    IF b.expected_row_count IS NOT NULL AND b.expected_row_count <> v_actual THEN
        RAISE EXCEPTION 'water_polygons loader: expected_row_count (%) <> actual (%)', b.expected_row_count, v_actual;
    END IF;
    IF EXISTS (
        SELECT 1 FROM import_work.water_polygon_rows AS r
        WHERE r.import_batch_id = b.import_batch_id
          AND r.source_snapshot_version IS DISTINCT FROM b.source_snapshot_version
    ) THEN
        RAISE EXCEPTION 'water_polygons loader: work-row snapshot_version mismatch vs batch';
    END IF;
    RAISE NOTICE 'water_polygons_loader [5%%] precheck ok batch=% dry_run=% actual=% safe_new=% safe_update=% other_ignored=% elapsed_ms=%',
...
DROP TABLE IF EXISTS water_polygons_loader_work;
CREATE TEMP TABLE water_polygons_loader_work AS
SELECT
    ...
    END AS geom_ready,
    coalesce(NULL::numeric, ST_Area(r.geom::geography)::numeric) AS area_m2,
    coalesce(r.source_refs, '{}'::jsonb) AS source_refs,
    coalesce(r.normalized_data, '{}'::jsonb) AS normalized_data,
    r.core_selection_reason
FROM import_work.water_polygon_rows AS r
JOIN water_polygons_loader_batch AS b ON b.import_batch_id = r.import_batch_id
WHERE r.classification IN ('safe_new', 'safe_update')
ORDER BY r.id
LIMIT greatest(
    (SELECT sample_limit FROM water_polygons_loader_params),
    CASE WHEN (SELECT sample_limit FROM water_polygons_loader_params) = 0 THEN 1000000000 ELSE 0 END
);

CREATE INDEX ON water_polygons_loader_work (id);
CREATE INDEX ON water_polygons_loader_work (identity_key);

DO $$ BEGIN RAISE NOTICE 'water_polygons_loader [15%%] work rows ready n=%', (SELECT count(*) FROM water_polygons_loader_work); END $$;

UPDATE import_work.water_polygon_rows AS r
...
FROM water_polygons_loader_work AS w WHERE r.id = w.id;

DROP TABLE IF EXISTS water_polygons_loader_core_by_identity;
CREATE TEMP TABLE water_polygons_loader_core_by_identity AS
...
FROM core.core_map_water_polygons AS c
...
CREATE INDEX ON water_polygons_loader_core_by_identity (identity_key);

DROP TABLE IF EXISTS water_polygons_loader_core_by_id;
CREATE TEMP TABLE water_polygons_loader_core_by_id AS
SELECT c.id, c.name, c.class_code, c.geom, c.is_verified, c.verification_status, c.source_refs
FROM core.core_map_water_polygons AS c
WHERE c.deleted_at IS NULL AND coalesce(c.is_active, true);
CREATE INDEX ON water_polygons_loader_core_by_id (id);

DO $$ BEGIN RAISE NOTICE 'water_polygons_loader [30%%] building plan (match + classify)...'; END $$;

DROP TABLE IF EXISTS water_polygons_loader_plan;
CREATE TEMP TABLE water_polygons_loader_plan (
...
WITH dup AS (
    SELECT identity_key FROM water_polygons_loader_work
...
matched AS (
    ...
    FROM water_polygons_loader_work AS w
    LEFT JOIN water_polygons_loader_core_by_id AS c_by_id
        ON w.target_core_id IS NOT NULL AND c_by_id.id = w.target_core_id
    LEFT JOIN water_polygons_loader_core_by_identity AS c_by_key
        ON w.identity_key IS NOT NULL AND c_by_key.identity_key = w.identity_key AND c_by_id.id IS NULL
)
INSERT INTO water_polygons_loader_plan (
...
BEGIN
    ...
    INTO v_fail, v_sample FROM water_polygons_loader_plan WHERE action = 'fail';
    SELECT count(*) FILTER (WHERE action = 'insert'),
           count(*) FILTER (WHERE action = 'update'),
           count(*) FILTER (WHERE action = 'skip'),
           count(*)
    INTO v_insert, v_update, v_skip, v_total FROM water_polygons_loader_plan;
    RAISE NOTICE 'water_polygons_loader [45%%] plan ready total=% insert=% update=% skip=% fail=%',
        v_total, v_insert, v_update, v_skip, v_fail;
    IF v_fail > 0 THEN
        RAISE EXCEPTION 'water_polygons loader: % failed row(s); sample: %', v_fail, v_sample;
    END IF;
END $$;

DROP TABLE IF EXISTS water_polygons_loader_result;
CREATE TEMP TABLE water_polygons_loader_result (
...
INSERT INTO water_polygons_loader_result (skipped, failed, core_before)
SELECT
    (SELECT count(*) FROM water_polygons_loader_plan WHERE action = 'skip'),
    (SELECT count(*) FROM water_polygons_loader_plan WHERE action = 'fail'),
    (SELECT count(*) FROM core.core_map_water_polygons WHERE deleted_at IS NULL AND coalesce(is_active, true));
...
BEGIN
    SELECT count(*) INTO v_total FROM water_polygons_loader_plan WHERE action = 'insert';
    RAISE NOTICE 'water_polygons_loader [50%%] insert start total=% chunk=%', v_total, v_chunk_size;
    IF v_total = 0 THEN
        RAISE NOTICE 'water_polygons_loader [70%%] insert skipped (0 rows)';
    ELSE
        LOOP
            SELECT coalesce(array_agg(work_id ORDER BY work_id), ARRAY[]::bigint[]) INTO v_ids
            FROM (
                SELECT work_id FROM water_polygons_loader_plan WHERE action = 'insert'
                ORDER BY work_id OFFSET v_done LIMIT v_chunk_size
            ) s;
            EXIT WHEN coalesce(array_length(v_ids, 1), 0) = 0;

            WITH ins AS (
                INSERT INTO core.core_map_water_polygons (
                ...
                SELECT
                    ...
                    coalesce(w.normalized_data, '{}'::jsonb) || jsonb_build_object(
                        ...
                        'loader', 'import_work.water_polygons_safe_loader'
                    ),
                    w.source_refs,
                    'unverified', false, true, now(), now(), NULL::timestamptz
                FROM water_polygons_loader_work AS w
                JOIN water_polygons_loader_plan AS p ON p.work_id = w.id
            ...
            name_src AS (
                ...
                FROM ins AS i
                JOIN water_polygons_loader_work AS w
                  ON system.pipeline_osm_identity_key(w.external_id)
                   = system.pipeline_osm_identity_key(i.external_id)
            ),
            
            del_names AS (
                DELETE FROM core.core_map_water_polygon_names AS n
                USING name_src AS s
                WHERE n.water_polygon_id = s.entity_id
            ...
            ins_en AS (
                INSERT INTO core.core_map_water_polygon_names (
                    water_polygon_id, name, language_code, script_code,
            ...
            UPDATE water_polygons_loader_result SET names_written = names_written + coalesce(v_names, 0);
            v_done := v_done + coalesce(v_chunk_n, 0);
            UPDATE water_polygons_loader_result SET inserted = v_done;
            v_pct := least(70, 50 + ((v_done * 20) / greatest(v_total, 1))::int);
            RAISE NOTICE 'water_polygons_loader [% %%] insert progress %/% (+%)', v_pct, v_done, v_total, coalesce(v_chunk_n, 0);
            EXIT WHEN v_done >= v_total;
        END LOOP;
    END IF;

    RAISE NOTICE 'water_polygons_loader [75%%] updates starting...';

    WITH upd AS (
        UPDATE core.core_map_water_polygons AS c
        SET
            name = w.name_ready,
            ...
            normalized_data = coalesce(c.normalized_data, '{}'::jsonb)
                ...
                || jsonb_build_object('loader', 'import_work.water_polygons_safe_loader'),
    ...
    UPDATE water_polygons_loader_result
    ...
    RAISE NOTICE 'water_polygons_loader [90%%] updates complete';
END $$;

UPDATE water_polygons_loader_result SET core_after = (
    SELECT count(*) FROM core.core_map_water_polygons WHERE deleted_at IS NULL AND coalesce(is_active, true)
);

DO $$ BEGIN RAISE NOTICE 'water_polygons_loader [95%%] writing publish batch + batch summary...'; END $$;
...
WITH pub AS (
    ...
    SELECT
        format('import_work_water_polygons:%s:%s', b.batch_code, to_char(clock_timestamp(), 'YYYYMMDDHH24MISSUS')),
        ...
        CASE WHEN p.dry_run THEN 'dry_run (will roll back if wrapper rolls back)'
             ELSE 'import_work water_polygons safe loader' END,
        b.source_snapshot_version,
        (SELECT count(*) FROM water_polygons_loader_plan)::int,
        (SELECT (inserted + updated)::int FROM water_polygons_loader_result),
        (SELECT failed::int FROM water_polygons_loader_result),
        (SELECT skipped::int FROM water_polygons_loader_result),
        jsonb_build_object(
            'loader', 'import_work.water_polygons_safe_loader',
            'import_batch_id', b.import_batch_id,
            'batch_code', b.batch_code,
            'entity_family', 'water_polygons',
            'dry_run', p.dry_run,
            'allowlist', jsonb_build_array(
                'name','class_code','geom','normalized_data','source_refs','core_map_water_polygon_names'
            ),
            'counts', (SELECT to_jsonb(r) - 'publish_batch_id' FROM water_polygons_loader_result r),
            'skip_reasons', (
                SELECT coalesce(jsonb_object_agg(skip_reason, cnt), '{}'::jsonb)
                FROM (
                    SELECT skip_reason, count(*) AS cnt FROM water_polygons_loader_plan
        ...
        CASE WHEN p.dry_run THEN NULL ELSE now() END
    FROM water_polygons_loader_batch AS b
    CROSS JOIN water_polygons_loader_params AS p
    RETURNING id
)
UPDATE water_polygons_loader_result SET publish_batch_id = (SELECT id FROM pub);

UPDATE import_work.import_batches AS b
SET
    status = CASE WHEN (SELECT dry_run FROM water_polygons_loader_params) THEN b.status ELSE 'applied' END,
    validation_status = CASE WHEN (SELECT dry_run FROM water_polygons_loader_params) THEN b.validation_status ELSE 'passed' END,
    validation_summary = coalesce(b.validation_summary, '{}'::jsonb) || jsonb_build_object(
        'water_polygons_safe_loader', (SELECT to_jsonb(r) FROM water_polygons_loader_result r),
        'dry_run', (SELECT dry_run FROM water_polygons_loader_params),
        'ran_at', now()
    ),
    updated_at = now()
FROM water_polygons_loader_batch AS lb
WHERE b.id = lb.import_batch_id;

DO $$
DECLARE r water_polygons_loader_result%ROWTYPE; v_dry boolean;
BEGIN
    SELECT * INTO r FROM water_polygons_loader_result;
    SELECT dry_run INTO v_dry FROM water_polygons_loader_params;
    RAISE NOTICE 'water_polygons_loader [100%%] done dry_run=% inserted=% updated=% skipped=% failed=% names=% core_before=% core_after=% delta=% duration_ms=%',
...
SELECT 'water_polygons_safe_loader_result' AS section, p.dry_run, b.batch_code,
...
FROM water_polygons_loader_result AS r
CROSS JOIN water_polygons_loader_params AS p
CROSS JOIN water_polygons_loader_batch AS b;

SELECT 'water_polygons_safe_loader_plan' AS section, action, count(*) AS n,
...
FROM water_polygons_loader_plan GROUP BY action ORDER BY action;
