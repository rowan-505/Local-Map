#!/usr/bin/env bash
# water_lines safe loader — contract-aware runner.
#
# Usage:
#   ./run_water_lines_safe_loader.sh --target local|production --batch-code <code> [--dry-run|--apply] \
#       [--confirmation 'APPLY water_lines <batch_id>'] [--sample-limit N] [--skip-cleanup] [--env-file path]
#
set -euo pipefail
...
usage() {
  cat <<'EOF'
usage: run_water_lines_safe_loader.sh --target local|production --batch-code <code> [options]
...
Production apply only:
  --confirmation 'APPLY water_lines <batch_id>'
...
safe_loader_preflight "${TARGET}" "${MODE}" "water_lines" "${BATCH_CODE}" "${CONFIRMATION}"
...
LOG_FILE="${LOG_DIR}/water_lines_loader_${SAFE_LOADER_TARGET}_${SAFE_BATCH}_${MODE}_${RUN_TS}.log"

echo ""
echo "=== water_lines_safe_loader ==="
...
stdbuf -oL -eL psql "${SAFE_LOADER_DATABASE_URL}" \
  ...
  -f "${SCRIPT_DIR}/water_lines_safe_loader.sql" \
  2> >(
