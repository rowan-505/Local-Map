-- Preload Kyauktan named water polygons (snapshot 4 safe_new) into import_work.
-- Yangon downtown sample has 0 eligible water polygons; this proves the loader.
...
\if :{?batch_code}
\else
\set batch_code 'water_polygons_kyauktan_safe_2026_07_23'
\endif
...
CREATE TEMP TABLE IF NOT EXISTS kyauktan_water_polygon_export (
    external_id text,
    classification text,
    canonical_name text,
    name_en text,
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM kyauktan_water_polygon_export) THEN
        RAISE EXCEPTION 'kyauktan_water_polygon_export is empty — run COPY first';
    END IF;
    IF EXISTS (SELECT 1 FROM kyauktan_water_polygon_export WHERE classification = 'pmtiles_only') THEN
        RAISE EXCEPTION 'preload refused: pmtiles_only rows present';
    END IF;
    IF EXISTS (
        SELECT 1 FROM kyauktan_water_polygon_export
...
INSERT INTO import_work.import_batches (
    ...
    btrim(:'batch_code'), 'water_polygons', :'snapshot_id'::bigint, btrim(:'snapshot_version'),
...
DELETE FROM import_work.water_polygon_rows r
...
FROM kyauktan_water_polygon_export AS e
...
SET
    status = 'loaded',
    expected_row_count = (SELECT count(*) FROM import_work.water_polygon_rows r WHERE r.import_batch_id = b.id),
    loaded_row_count = (SELECT count(*) FROM import_work.water_polygon_rows r WHERE r.import_batch_id = b.id),
    loaded_at = now(),
    updated_at = now()
WHERE b.batch_code = btrim(:'batch_code');

SELECT 'water_polygons_preload' AS section, b.id AS batch_id, b.batch_code, b.status,
