...
--
-- Requires:
--   -v csv_path='/absolute/path/to/_yangon_safe_settlements.csv'
--   -v batch_code='places_yangon_settlements_safe_2026_07_23'
--
...
\if :{?batch_code}
\else
\set batch_code 'places_yangon_settlements_safe_2026_07_23'
\endif

-- Caller must replace __CSV_PATH__ before running, or use the shell wrapper.
BEGIN;

CREATE TEMP TABLE yangon_safe_export (
    external_id text,
    import_class text,
    primary_name text,
    display_name text,
...
\copy yangon_safe_export FROM '/Users/nyihtet/Documents/Projects/Core-Map/tools/data-pipeline/import-work/reports/_yangon_safe_settlements.csv' WITH (FORMAT csv, HEADER true)
...
CREATE TEMP TABLE yangon_safe_resolved AS
SELECT
    e.*,
...
FROM yangon_safe_resolved AS r
...
CREATE TEMP TABLE yangon_load_ready AS
SELECT *
FROM yangon_safe_resolved
...
SELECT
    ...
    import_class,
    count(*) AS n
FROM yangon_load_ready
GROUP BY import_class
ORDER BY import_class;

SELECT 'load_ready_total' AS section, count(*) AS n FROM yangon_load_ready;
...
FROM yangon_safe_resolved
WHERE NOT EXISTS (
    SELECT 1 FROM yangon_load_ready lr WHERE lr.external_id = yangon_safe_resolved.external_id
)
GROUP BY 1, 2
ORDER BY n DESC;
...
INSERT INTO import_work.import_batches (
    ...
    'places',
    9,
    'osm_myanmar_2026_07_21_yangon_city_v1',
    'loading',
    (SELECT count(*) FROM yangon_load_ready),
    'not_started',
    'Yangon settlements safe load 2026-07-23'
)
...
SELECT
    ...
    coalesce(r.source_refs::jsonb, '{}'::jsonb)
        || jsonb_build_object(
            ...
            'gate', 'yangon_settlements_pilot'
        ),
    nullif(btrim(r.source_hash), ''),
    'passed',
    jsonb_build_object(
        'preload', 'yangon_settlements_preload_and_copy',
        'mapped_category_code', r.mapped_category_code
    )
FROM yangon_load_ready AS r
...
SET
    ...
    validation_summary = jsonb_build_object(
        'preload_at', now(),
        'source_snapshot_version', 'osm_myanmar_2026_07_21_yangon_city_v1',
        'classified_safe_total', (SELECT count(*) FROM yangon_safe_resolved),
        'load_ready_total', (SELECT count(*) FROM yangon_load_ready),
        'excluded_in_ir', (SELECT count(*) FROM yangon_safe_resolved WHERE in_import_review),
        'excluded_unsupported_category', (SELECT count(*) FROM yangon_safe_resolved WHERE category_id IS NULL),
        'safe_new', (SELECT count(*) FROM yangon_load_ready WHERE import_class = 'safe_new'),
        'safe_update', (SELECT count(*) FROM yangon_load_ready WHERE import_class = 'safe_update')
    )
WHERE b.batch_code = :'batch_code';
