...
SELECT 1/CASE WHEN EXISTS(SELECT 1 FROM :"staging_schema".staging_water_line_candidates s,direct_export_context x
...
\copy (
 SELECT s.import_class classification,s.id local_staging_id,
  ...
  ST_AsEWKT(ST_Multi(ST_CollectionExtract(ST_MakeValid(s.geom),2)))geom_ewkt,
  s.source_refs,s.normalized_data
 FROM :"staging_schema".staging_water_line_candidates s,direct_export_context x
...
\copy (
 SELECT 'water_lines'entity_family,s.id local_staging_id,s.external_id,s.import_class,
...
SELECT import_class,count(*)n FROM :"staging_schema".staging_water_line_candidates s,direct_export_context x
