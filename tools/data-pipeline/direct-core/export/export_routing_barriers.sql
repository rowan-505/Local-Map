...
SELECT 1/CASE WHEN EXISTS(SELECT 1 FROM :"staging_schema".staging_routing_barrier_candidates s,direct_export_context x
...
\copy (
 SELECT s.import_class classification,s.id local_staging_id,
  system.pipeline_osm_identity_key(s.external_id)external_id,s.barrier_type,
  (s.normalized_data->>'core_street_id')::bigint core_street_id,
  ST_AsEWKT(coalesce(s.point_geom,
   ST_PointOnSurface(ST_MakeValid(s.geom)))::geometry(Point,4326))point_ewkt,
  coalesce(s.access_tags,'{}'::jsonb)access_tags,s.source_refs,s.normalized_data
 FROM :"staging_schema".staging_routing_barrier_candidates s,direct_export_context x
...
\copy (
 SELECT 'routing_barriers'entity_family,s.id local_staging_id,s.external_id,s.import_class,
...
SELECT import_class,count(*)n FROM :"staging_schema".staging_routing_barrier_candidates s,direct_export_context x
