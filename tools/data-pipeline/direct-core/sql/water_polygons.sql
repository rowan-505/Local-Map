 ...
 array_remove(ARRAY[
  ...
  CASE WHEN s.classification='safe_new'AND c.id IS NOT NULL AND(
   (coalesce(s.name_my,s.name_en,s.name_und) IS NOT NULL AND NOT EXISTS (
      SELECT 1 FROM core.core_water_polygon_names n
      WHERE n.water_polygon_id=c.id AND n.name=coalesce(s.name_my,s.name_en,s.name_und)))
...
WITH ins AS(
 INSERT INTO core.core_water_polygons(external_id,water_class_id,geom,source_registry_id,source_snapshot_id,source_feature_type,source_feature_id,region_code,source_refs,normalized_data,
  is_active,verification_status)
 SELECT s.external_id,s.water_class_id,s.geom::geometry(MultiPolygon,4326),
...
WITH upd AS(
 UPDATE core.core_water_polygons c SET water_class_id=s.water_class_id,
