...
VALUES
    ('buildings', 'staging_building_candidates', 'buildings', 'core_buildings', NULL,
    ...
    ('landuse', 'staging_landuse_candidates', 'landuse', 'core_land_areas', NULL,
     'CASE WHEN s.geom IS NOT NULL THEN ST_AsGeoJSON(s.geom)::jsonb END',
     's.class_code::text', 's.canonical_name::text', NULL, NULL, NULL, NULL,
     's.geom IS NOT NULL',
     'jsonb_build_object(''name'', s.canonical_name, ''land_area_class_id'', to_jsonb(s) -> ''land_area_class_id'', ''centroid_geojson'', CASE WHEN s.geom IS NOT NULL THEN ST_AsGeoJSON(ST_Centroid(s.geom))::jsonb END)'),
    ('water_lines', 'staging_water_line_candidates', 'water_lines', 'core_water_lines', NULL,
     'CASE WHEN s.geom IS NOT NULL THEN ST_AsGeoJSON(s.geom)::jsonb END',
     'coalesce(nullif(trim(s.class_code::text), ''''), nullif(trim(s.canonical_name::text), ''''))',
     's.canonical_name::text', NULL, NULL, NULL, NULL,
     's.geom IS NOT NULL',
     'jsonb_build_object(''name'', s.canonical_name, ''water_class_id'', to_jsonb(s) -> ''water_class_id'')'),
    ('water_polygons', 'staging_water_polygon_candidates', 'water_polygons', 'core_water_polygons', NULL,
     'CASE WHEN s.geom IS NOT NULL THEN ST_AsGeoJSON(s.geom)::jsonb END',
     'coalesce(nullif(trim(s.class_code::text), ''''), nullif(trim(s.canonical_name::text), ''''))',
     's.canonical_name::text', NULL, NULL, NULL, NULL,
     's.geom IS NOT NULL',
     'jsonb_build_object(''name'', s.canonical_name, ''water_class_id'', to_jsonb(s) -> ''water_class_id'', ''centroid_geojson'', CASE WHEN s.geom IS NOT NULL THEN ST_AsGeoJSON(ST_Centroid(s.geom))::jsonb END)'),
