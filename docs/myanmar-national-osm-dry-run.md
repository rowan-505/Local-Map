...
**Hard guards:** no Supabase `core` writes, no Import Review upload, no safe-loader apply.
...
National PBF (Geofabrik myanmar-260721)
  ...
  → import_class buckets (Stage 08b / 18)
  → reports only
```
...
2. **Settlements-only admin assign** in Stage 05 places (POIs skip `pipeline_assign_admin_area_for_point`).
