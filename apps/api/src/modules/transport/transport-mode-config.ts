/**
 * Shared Transport mode config (API side).
 *
 * Small, generic per-mode defaults so a single route editor flow can support
 * bus, train, and ferry without mode-specific code paths. Intentionally tiny:
 * no plugin system, no behaviour beyond defaults + labels.
 *
 * Mirror of `apps/dashboard/src/features/transport/modeConfig.ts` — keep the two
 * files identical.
 */
