...
export type ImportReviewBuildingListItem = {
    ...
    /** Present on some detail/history payloads when promoted. */
...
export type ImportReviewRoadsListParams = ImportReviewBuildingsListParams & {
    promotion_status?: string;
    class_code?: string;
    include_promoted?: boolean;
    retry_needed?: boolean;
...
export type PatchImportReviewCandidateColumnsBody = ImportReviewEnvelopeQuery & {
    fields: Record<string, unknown>;
    review_note?: string | null;
...
export type PatchImportReviewRoadColumnsBody = PatchImportReviewCandidateColumnsBody & {
    fields: ImportReviewRoadReviewOverridesLeaf;
};
...
export type PostImportReviewBuildingsBulkBody = ImportReviewEnvelopeQuery & {
    review_decision: ImportReviewDecision;
    review_status?: ImportReviewReviewStatus;
    review_note?: string | null;
    force?: boolean;
...
async function apiFetchInternal<T>(
    ...
    if (!response.ok) {
        const contentType = response.headers.get("content-type") ?? "";
        const isRoadCandidateColumnPatchPath =
            /\/import-review\/roads\/\d+$/.test(path);
        ...
        if (contentType.includes("application/json")) {
            ...
            const {
                ...
                ImportReviewRoadOverridesSaveError,
            } = await import("@/src/features/import-review/api/importReviewApiErrors");

            if (isRoadCandidateColumnPatchPath) {
                const roadOverrideIssues = parseImportReviewRoadOverridesSaveIssues(data);
                if (
                    roadOverrideIssues &&
                    (roadOverrideIssues.errors.length > 0 || roadOverrideIssues.requiresAcknowledgement)
                ) {
                    const formatted = formatImportReviewApiErrorBody(
                        data,
                    ...
                    throw new ImportReviewRoadOverridesSaveError(
                        response.status,
                        code,
                        roadOverrideIssues,
                        formatted
            ...
            if (response.status === 409 && isImportReviewApiPath(path)) {
            ...
            const legacyMessage = formatImportReviewApiErrorBody(
                data,
                `Request failed with status ${response.status}`,
                { includeTechnicalDetails: true }
            );
            if (legacyMessage !== `Request failed with status ${response.status}`) {
...
export function patchImportReviewBuildingColumns(id: string, body: PatchImportReviewCandidateColumnsBody) {
    return apiFetch<ImportReviewBuildingListItem>(`/api/import-review/buildings/${id}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json",
        },
...
export function patchImportReviewFamilyColumns(
    family: string,
    id: string,
    body: PatchImportReviewCandidateColumnsBody
) {
    const familyPath = encodeURIComponent(resolveImportReviewApiFamily(family));
    return apiFetch<ImportReviewBuildingListItem>(`/api/import-review/${familyPath}/${id}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json",
        },
...
export function getImportReviewRoadDryRunSummary(
    params: ImportReviewEnvelopeQuery,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewRoadDryRunSummaryResponse>(
        "/api/import-review/roads/dry-run-summary",
        { method: "GET", ...fetchInit },
        normalizedImportReviewUrlParams(params as Record<string, QueryValue>)
    );
}
...
function wireRoadColumnFields(fields: ImportReviewRoadReviewOverridesLeaf): ImportReviewRoadReviewOverridesLeaf {
    const out = { ...fields };
    const rc = out.road_class_id as unknown;
    if (typeof rc === "bigint") {
        out.road_class_id = rc.toString();
    }
    return out;
}

export function patchImportReviewRoadColumns(id: string, body: PatchImportReviewRoadColumnsBody) {
    const payload: PatchImportReviewRoadColumnsBody = {
        ...body,
        fields: wireRoadColumnFields(body.fields),
    };

    return apiFetch<ImportReviewBuildingListItem>(`/api/import-review/roads/${id}`, {
        method: "PATCH",
        headers: {
            "Content-Type": "application/json",
        },
...
export type ImportReviewPublishBatchSummary = {
    ...
    public_id: string;
    batch_name: string;
    /** Families selected at batch creation when returned by detail API. */
    entity_families?: string[];
    status: string;
...
/** @deprecated Legacy batch-eligibility endpoint shape */
...
export type ImportReviewPromotionEligibilityParams = {
    review_batch_id: string | number;
    families: string[];
    include_warnings?: boolean;
};

export type ImportReviewPromotionEligibilityBucket =
    | "ready"
    | "warnings"
    | "blocked"
    | "batched"
...
export type ImportReviewPromotionEligibilityDetailsSortBy = "id" | "updated_at" | "confidence_score";

export type ImportReviewPromotionEligibilityDetailsParams = {
    review_batch_id: string | number;
    family: string;
    bucket: ImportReviewPromotionEligibilityBucket;
    include_warnings?: boolean;
    ...
    search?: string;
    reason_code?: string;
    sort_by?: ImportReviewPromotionEligibilityDetailsSortBy;
    sort_order?: "asc" | "desc";
};
...
export type ImportReviewCreatePublishBatchResult = {
    ...
    /** @deprecated Use candidates_marked_batched / by_family */
...
export type PostImportReviewPromotionBatchBody = {
    review_batch_id: string | number;
    mode?: "selected" | "all_ready";
    families?: string[];
    candidate_ids_by_family?: Record<string, Array<string | number>>;
    filters?: {
        review_decision?: string;
        include_warnings?: boolean;
    };
    ...
    max_items?: number;
    limit_per_family?: Record<string, number>;
    /** @deprecated Use `families` — still accepted by API preprocess */
    entity_families?: string[];
};

export type PostImportReviewPromotionBatchValidateBody = {
    confirm_large_batch?: boolean;
    allow_high_risk_families?: boolean;
    mixed_high_risk_confirm?: boolean;
};
...
export function getImportReviewPromotionEligibility(
    params: ImportReviewPromotionEligibilityParams,
    fetchInit?: Pick<RequestInit, "signal">
) {
    const url = new URL("/api/import-review/promotion/eligibility", `${getApiBaseUrl()}/`);
    url.searchParams.set("review_batch_id", String(params.review_batch_id));
    const families = [...params.families].sort();
    url.searchParams.set("families", families.join(","));
    if (params.include_warnings) {
        url.searchParams.set("include_warnings", "true");
    }
    return apiFetch<ImportReviewPromotionEligibilityResponse>(url.pathname + url.search, {
        method: "GET",
        ...fetchInit,
    });
}

export function getImportReviewPromotionEligibilityDetails(
    params: ImportReviewPromotionEligibilityDetailsParams,
    fetchInit?: Pick<RequestInit, "signal">
) {
    const url = new URL("/api/import-review/promotion/eligibility/details", `${getApiBaseUrl()}/`);
    url.searchParams.set("review_batch_id", String(params.review_batch_id));
    ...
    if (params.reason_code?.trim()) {
        url.searchParams.set("reason_code", params.reason_code.trim());
    }
    if (params.sort_by) {
    ...
    return apiFetch<ImportReviewPromotionEligibilityDetailsResponse>(url.pathname + url.search, {
        method: "GET",
        ...fetchInit,
    });
}

/** @deprecated Prefer {@link getImportReviewPromotionEligibility} */
...
export type ImportReviewPublishBatchValidationResultSummary = {
    outcome: "passed" | "partial" | "blocked" | "ready";
    can_promote: boolean;
    ...
    /** @deprecated Use by_entity / selected_entity_families */
...
export type ImportReviewRoadPromotionGateId =
    | "env_enabled"
    | "env_bulk_enabled"
...
export type ImportReviewRoadPromotionGateCheck = {
    id: ImportReviewRoadPromotionGateId;
    label: string;
    satisfied: boolean;
    detail: string;
    helper?: string;
...
export type ImportReviewRoadPromotionGatesResult = {
    applies: boolean;
    can_promote: boolean;
    ...
    env_enabled: boolean;
    gates: ImportReviewRoadPromotionGateCheck[];
    primary_blocker: ImportReviewRoadPromotionGateId | null;
...
export type ImportReviewPublishBatchProgressResponse = {
    ...
    road_promotion_gates?: ImportReviewRoadPromotionGatesResult | null;
    dry_run_result?: {
        status: string;
        checked_at?: string;
        total?: number;
...
export type PostImportReviewPromotionReleaseStaleBatchedBody = {
    review_batch_id: number;
    families?: string[];
    dry_run?: boolean;
};
...
const VALIDATABLE_BATCH_STATUSES = new Set([
    "draft",
    "failed",
    "cancelled",
    "validated",
...
export type ImportReviewPublishBatchDryRunResponse = {
    ...
    summary: {
        dry_run_result: {
            ...
            would_insert_count?: number;
            would_update_count?: number;
            sample_errors?: ImportReviewPublishBatchDryRunResponse["sample_errors"];
        };
        batch_status?: string;
        message?: string;
    };
...
export function postImportReviewPromotionBatchDryRun(
    id: string,
    body: { confirm_large_batch?: boolean } = {},
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewPublishBatchDryRunResponse>(
        `/api/import-review/promotion/batches/${id}/dry-run`,
        {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
...
export function postImportReviewPromotionBatchCancelValidation(
    id: string,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewStartPublishBatchValidationResponse>(
        `/api/import-review/promotion/batches/${id}/cancel-validation`,
        { method: "POST", ...fetchInit }
    );
}

export function postImportReviewPromotionBatchResetValidation(
    id: string,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewStartPublishBatchValidationResponse>(
        `/api/import-review/promotion/batches/${id}/reset-validation`,
        { method: "POST", ...fetchInit }
    );
}

export function postImportReviewPromotionBatchResume(
    id: string,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<{ batch_id: string; action: string; message: string }>(
        `/api/import-review/promotion/batches/${id}/resume`,
        { method: "POST", ...fetchInit }
    );
}

export function postImportReviewPromotionBatchCancelPromotion(
    id: string,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewStartPublishBatchPromotionResponse>(
        `/api/import-review/promotion/batches/${id}/cancel-promotion`,
        { method: "POST", ...fetchInit }
    );
}

export function postImportReviewPromotionBatchResetPromotion(
    id: string,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewStartPublishBatchPromotionResponse>(
        `/api/import-review/promotion/batches/${id}/reset-promotion`,
        { method: "POST", ...fetchInit }
    );
}
...
export function postImportReviewPromotionBatchRetryFailedReady(
    id: string,
    body: PostImportReviewPromotionBatchRetryFailedReadyBody = {},
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewCreateRetryPublishBatchResult>(
        `/api/import-review/promotion/batches/${id}/retry-failed-ready`,
        {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
...
export function postImportReviewPromotionReleaseStaleBatched(
    body: PostImportReviewPromotionReleaseStaleBatchedBody,
    fetchInit?: Pick<RequestInit, "signal">
) {
    return apiFetch<ImportReviewReleaseStaleBatchedResponse>(
        "/api/import-review/promotion/release-stale-batched",
        {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
...
/** summary.road_dry_run block */
export type ImportReviewRoadDryRunSummaryBlock = {
    status: "passed" | "failed";
    checked_count: number;
    passed_count: number;
    failed_count: number;
...
/** POST/GET /promotion/batches/:id/road-dry-run response */
export type ImportReviewPromotionRoadDryRunSummaryResponse = {
    batch_id: string;
    road_dry_run: ImportReviewRoadDryRunSummaryBlock;
    routing_readiness_validation: ImportReviewRoadRoutingReadinessSummary;
};

/** Legacy detailed dry-run payload (optional on batch summary). */
...
export function postImportReviewPromotionBatchRoadDryRun(
    ...
    return apiFetch<ImportReviewPromotionRoadDryRunSummaryResponse>(
...
export type ImportReviewHistoryPublishBatchItemsParams = {
    /** Filter token or raw publish_status (failed, pending, success, …). */
    publish_status?: string;
    /** Alias for publish_status — same accepted tokens. */
    status?: string;
    entity_family?: string;
    limit?: number;
    offset?: number;
