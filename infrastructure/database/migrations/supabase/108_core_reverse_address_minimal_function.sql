-- Minimal raw reverse-address lookup for a clicked point.
-- Reuses the existing core.find_admin_area_for_point(p_geom, target_admin_level_code)
-- and the parent_id chain on core.core_admin_areas. Read-only: no writes, no plus_code.
-- Relies on existing GIST indexes (core_admin_areas geom, core_places point_geom,
-- core_streets geom); creates no tables and no new indexes.
...
begin
    ...
    -- 2b. Fallback: nearest active named street within 100m.
    if v_nearby_name is null then
        ...
        where s.is_active = true
          ...
          and ST_DWithin(s.geom::geography, v_point::geography, 100)
...
comment on function core.reverse_address_minimal(double precision, double precision) is
    'Minimal raw reverse address: nearest public place (<=300m) or active named street (<=100m), plus township/district/state_region/country resolved from core.find_admin_area_for_point + parent chain. English/canonical names; country falls back to Myanmar. Read-only, no plus_code.';
