-- Allow publish batches validated with some promotable and some blocked/warning items.

DO $$
BEGIN
    IF to_regclass('system.system_publish_batches') IS NULL THEN
        RAISE NOTICE 'Skipped partial status: system.system_publish_batches missing.';
        RETURN;
    END IF;
